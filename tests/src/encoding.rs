use opticrum_calculator::types::{
    CompressedPubkey, MatchArgs, MatchData, MatchStatus, OrderArgs, OrderData, MATCH_ARGS_LEN,
    MATCH_DATA_LEN, ORDER_ARGS_LEN, ORDER_DATA_LEN,
};
...
fn test_match_data_encoding() {
    let match_data = MatchData::new(0, 1.5, faker::ESCROW_BLOCKS, MatchStatus::Frozen);
    ...
    assert_eq!(parsed.status, MatchStatus::Frozen);
}
...
fn test_match_status_roundtrip() {
    for status in [
        ...
        let parsed = MatchStatus::from_byte(byte).expect("valid status byte");
        assert_eq!(parsed, status);
    }
    assert!(MatchStatus::from_byte(0xFF).is_none());
}
...
fn test_complete_lifecycle_types() {
    ...
    assert_eq!(match_data.status, MatchStatus::Frozen);
}
