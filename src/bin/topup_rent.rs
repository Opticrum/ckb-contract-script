...
pub async fn main() -> eyre::Result<()> {
    ...
    let match_index: usize = 0; // pick which scanned match to top up
    let capacity_delta: i64 = (10000u64 * ONE_CKB) as i64; // add 10,000 CKB
    ...
    let topup = update_match_buyer::<RpcClient>(
        buyer_address.clone(),
        match_info.clone(),
        current_xudt,
        capacity_delta,
    ...
    let (tx, _) = TransactionCalculator::new(vec![prepare, topup, balance])
        .new_skeleton(&rpc)
        .await?;
