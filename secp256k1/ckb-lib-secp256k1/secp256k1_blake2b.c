 ...
 */
int ckb_secp256k1_custom_load_data(void *data, uint8_t *script_data_hash) {
  if (script_data_hash == NULL) {
      script_data_hash = ckb_secp256k1_data_hash;
  }
  size_t index = SIZE_MAX;
  int ret = ckb_look_for_dep_with_hash(script_data_hash, &index);
  if (ret != CKB_SUCCESS) {
    return ret;
  }
...
int validate_secp256k1_blake2b_sighash_all(uint8_t *output_public_key_hash, uint8_t *script_data_hash) {
  ...
  ret = ckb_secp256k1_custom_load_data(secp_data, script_data_hash);
  if (ret != 0) {
    return ret;
  }
